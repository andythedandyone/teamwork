...
// Emails : used to store emails inside client struct
type Emails struct {
	email string
}
	...
	for {
		...
		if domain == "" {
			...
			if len(clientList) == 0 {
				...
				clientSingle.amount++
				clientSingle.emails = append(clientSingle.emails, data[2])
				...
				for i, thisClient := range clientList {
					if thisClient.domain != domain && len(clientList)-1 == i {
						...
						x.domain = domain
						x.amount++
						x.emails = append(x.emails, data[2])
						clientList = append(clientList, x)
					} else if thisClient.domain == domain {
						clientList[i].amount++
						clientList[i].emails = append(clientList[i].emails, data[2])
						break
					}
				}
			}
